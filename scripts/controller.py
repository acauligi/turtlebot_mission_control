...
class Controller:
    ...
    def __init__(self):
		...
		self.pose_goal = None
		self.path_list = None
		rospy.Subscriber('/turtlebot_mission_control/tag_pose', Float32MultiArray, self.poseCallback) #final pose to be acheived in front of apriltag because now care about orientation
        rospy.Subscriber('/turtlebot_mission_control/path_goal', Path, self.pathCallback) #a list of nodes computed using a*
		
	def poseCallback(self, msg)
		self.pose_goal = (msg.data[0],msg.data[1],msg.data[2]) #for final pose in front of desired apriltag
		
	def pathCallback(self, pathlist)
		self.path_list = pathlist #hopefully a list but possibly some convoluted multiarray
    ...
    def get_ctrl_output(self):
        ...
        #####warning, there is a chance robot may get stuck in loop if strays from path on a sharp turn
        #the difficult part is choosing the right point on path to target, then just used code from hw1
        distances = [np.linalg.norm(pathlist[i]-np.array((self.x,self.y))) for i in range(n)]
        ...
        if closest != len(pathlist)-1: #if the closest point on path is not the final goal target a point further along the path
            ...
            self.th_g=np.arctan2(pathlist[closest+1][1]-pathlist[closest][1],pathlist[closest+1][0]-pathlist[closest][0]) #make it parallel to current path we are on
